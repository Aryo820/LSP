...
if (isset($_POST['edit'])) {
    ...
    if (!empty($_FILES['photo']['name'])) {
        ...
        if (!in_array($img_ext, $ext)) {
            header("Location: ?page=profilsaya&edit=errorExtension");
            die;
        } else {
            if (file_exists('content/upload/' . $rowEdit['profile_picture'])) {
                unlink('content/upload/' . $rowEdit['profile_picture']);
            }
            $new_image_name = "Gambar" . $idEdit . "." . $img_ext;
            move_uploaded_file($_FILES['photo']['tmp_name'], 'content/upload' . $new_image_name);
    ...
    $queryEdit = mysqli_query($config, "UPDATE user SET username='$username', email='$email', password='$password',address='$address' WHERE id='$idEdit'");
    header("Location: ?page=profilsaya&edit=success");
    die;
}
